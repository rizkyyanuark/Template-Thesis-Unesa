...
\scriptsize
\begin{longtable}{|>{\raggedright\arraybackslash}p{0.26\textwidth}|>{\centering\arraybackslash}p{0.16\textwidth}|>{\raggedright\arraybackslash}p{0.46\textwidth}|}
    ...
    Berdasarkan evaluasi komparatif, studi ini membuktikan bahwa arsitektur \textit{GraphRAG} secara konsisten mengungguli \textit{Naive RAG} dan \textit{LightRAG} pada metrik \textit{Comprehensiveness, Diversity, Empowerment,} dan \textit{Directness}. Namun, studi tersebut masih menguji \textit{GraphRAG} pada domain medis, dan belum menguji hibrida yang mengabungkan pencarian vektor dan graf secara sinergis untuk menghasilkan informasi yang lebih akurat dan relevan.
    \\ \hline
    ...
    Studi ini membuktikan keunggulan pendekatan hibrida (\textit{Knowledge Graph} dan basis data vektor). Namun, penelitian tersebut dilakukan pada domain finansial (\textit{earning call transcripts}). Efektivitasnya pada domain literatur akademik yang memiliki topologi relasi kompleks seperti jaringan sitasi, kolaborasi penulis, dan bidang keilmuan masih menjadi celah riset (\textit{research gap}). Penelitian ini hadir untuk mengadopsi sekaligus menguji paradigma hibrida tersebut pada ekosistem literatur Infokom UNESA.
    \\ \hline

    % --- Studi 3 ---
    \textit{Knowledge Graph Generation and Application for Unstructured Data Using Data Processing Pipeline}                        &
    \cite{thushara2024}                                                                                                             &
    Penelitian ini menawarkan sebuah \textit{pipeline} otomatis untuk mengekstraksi \textit{Knowledge Graph} dari data tidak terstruktur dengan memanfaatkan model generatif \textit{REBEL}. Meskipun berhasil diimplementasikan, ketergantungan penuh pada model ekstraksi generatif tunggal menimbulkan beban komputasi tinggi dan masalah skalabilitas jika dihadapkan pada korpus akademik. Sebagai bentuk pembaruan, penelitian ini memodifikasi arsitektur tersebut dengan mengganti model generatif menjadi \textit{GLiNER} (berbasis \textit{zero-shot NER}) yang lebih ringan.
    \\ \hline
\end{longtable}
\endgroup
...
Komponen fundamental dari sebuah \textit{Knowledge Graph} adalah \textit{triplet} faktual yang terdiri dari \textit{Subjek-Predikat-Objek} atau \textit{Head-Relation-Tail}. \textit{Knowledge Graph} menyimpan pengetahuan terstruktur sebagai himpunan \textit{triplet} $T$ yang menghubungkan himpunan entitas $E$ dengan himpunan relasi $R$, yang diformulasikan secara berturut-turut pada persamaan \eqref{eq:kg-definition} dan \eqref{eq:triplet-format} sebagai berikut:

\begin{align}
    G         &= (E, R, T), \label{eq:kg-definition} \\
    (h, r, t) &\in T, \label{eq:triplet-format}
\end{align}
di mana $E$ adalah himpunan entitas (seperti individu atau objek), $R$ adalah himpunan relasi (seperti ``bagian\_dari''), $T$ adalah himpunan \textit{triplet} yang membentuk keseluruhan struktur graf tersebut \citep{Hogan_2021}, $h$ adalah entitas sumber atau \textit{head}, $t$ adalah entitas tujuan atau \textit{tail}, dan $r$ adalah relasi yang menghubungkan keduanya \citep{peng2023knowledge}. Contohnya dalam konteks akademik:
\begin{itemize}
...
Secara formal, tugas \textit{Knowledge Extraction} dapat diformulasikan sebagai fungsi pemetaan dari teks input \(\mathcal{T}\) menjadi representasi terstruktur \(\mathcal{S}\). Representasi ini terdiri dari himpunan entitas \(E\) dan relasi \(R\), sebagaimana dinyatakan dalam persamaan \eqref{eq:ie-mapping}:

\begin{align}
    \mathcal{S} &= f_{IE}(\mathcal{T}) \notag \\
                &= \{(e_i, r_{ij}, e_j) \mid e_i, e_j \in E, r_{ij} \in R\}, \label{eq:ie-mapping}
\end{align}
...
\begin{enumerate}[label=\textbf{\arabic*.}, leftmargin=*, itemsep=10pt]
    \item \textbf{\textit{Named Entity Recognition} (NER)}
          ...
          \hspace*{0.75cm} Sebagai langkah awal, \textit{Named Entity Recognition} (NER) bertugas mengidentifikasi dan mengklasifikasikan entitas penting dalam teks untuk menemukan himpunan entitas $E$ pada persamaan \eqref{eq:ie-mapping}. NER mendeteksi istilah relevan seperti nama orang, organisasi, atau terminologi ilmiah, lalu memberikan label kategori yang sesuai \citep{ZHAO2023225}.
...
\begin{figure}[!t]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1\textwidth]{assets/pics/arch transformer.png}}
...
Pada tahap inferensi, \textit{LLM} menghasilkan token secara \textit{autoregresif} berdasarkan distribusi probabilitas bersyarat pada persamaan \eqref{eq:llm-generation} berikut:
\begin{align}
    y_t &= \arg\max_{y} P_\theta(y \mid y_{<t}, x), \label{eq:llm-generation}
\end{align}
...
Meskipun demikian, \textit{LLM} memiliki dua keterbatasan fundamental yang menjadi tantangan besar dalam domain akademik. Pertama, \textit{LLM} memprediksi token keluaran berdasarkan probabilitas statistik, bukan berdasarkan pemahaman fakta. Akibatnya, model dapat menghasilkan informasi yang terdengar benar tetapi salah secara faktual — fenomena yang disebut halusinasi (\textit{hallucination}) \citep{lala2023paperqaretrievalaugmentedgenerativeagent}. Kedua, pengetahuan \textit{LLM} terbatas pada data yang digunakan saat pelatihan terakhirnya (\textit{knowledge cutoff}), sehingga model tidak mengetahui informasi yang terbit setelah masa pelatihan tersebut. Kedua keterbatasan ini menjadikan \textit{LLM} tidak dapat diandalkan secara mandiri untuk menjawab pertanyaan akademik yang menuntut keakuratan fakta dan kemutakhiran referensi.
...
Sebagaimana ditekankan oleh \citet{Huang_2025}, \textit{RAG} dirancang untuk mengatasi dua kelemahan utama \textit{LLM} yang telah diuraikan sebelumnya: risiko halusinasi dan keterbatasan \textit{knowledge cutoff}. Dengan menyediakan dokumen pendukung yang relevan sebagai konteks tambahan sebelum proses generasi, \textit{RAG} memastikan jawaban model berlandaskan pada fakta yang dapat diverifikasi, bukan sekadar prediksi statistik. Selain itu, karena basis dokumen dapat diperbarui secara independen tanpa melatih ulang model, \textit{RAG} juga mengatasi kendala keterbatasan pengetahuan \textit{LLM}. Secara umum, alur kerja standar RAG terdiri dari tiga tahapan utama, yaitu:
...
\begin{figure}[!t]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1\textwidth]{assets/pics/rag new.png}}
...
\begin{figure}[!t]
    \centering
    \makebox[\textwidth][c]{\includegraphics[width=1\textwidth]{assets/pics/framework graphrag.png}}
...
Tujuan \textit{GraphRAG} adalah menemukan jawaban optimal ($a^*$) untuk pertanyaan ($q$) berbasis Graf Pengetahuan ($\mathcal{G}$). Distribusi probabilitas target $p(a \mid q, \mathcal{G})$ dimodelkan melalui kolaborasi dua komponen utama, yaitu model \textit{graph retriever} $p_\theta(G \mid q, \mathcal{G})$ dan model \textit{answer generator} $p_\phi(a \mid q, G)$ dengan parameter latih $\theta$ dan $\phi$, yang secara berturut-turut diformulasikan pada persamaan \eqref{eq:graphrag_goal} dan \eqref{eq:graphrag_decomposition_approx} sebagai berikut:
\begin{align}
    a^*                      &= \operatorname*{arg\,max}_{a \in A} p(a \mid q, \mathcal{G}), \label{eq:graphrag_goal} \\
    p(a \mid q, \mathcal{G}) &\approx p_\phi(a \mid q, G^*)\, p_\theta(G^* \mid q, \mathcal{G}), \label{eq:graphrag_decomposition_approx}
\end{align}
di mana $A$ merepresentasikan himpunan seluruh kemungkinan respons yang dapat dihasilkan, $G^*$ adalah subgraf terpilih yang paling relevan, $p_\theta$ mewakili model \textit{retriever} yang mencari subgraf relevan, dan $p_\phi$ adalah model \textit{generator} yang menyusun jawaban berdasarkan subgraf tersebut.
