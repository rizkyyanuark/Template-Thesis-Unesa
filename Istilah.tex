...
% Glosarium istilah skripsi dan proposal.
%
% Cara memanggil istilah di dalam naskah:
% - \gls{label}   : nama istilah huruf kecil (contoh: \gls{triplet})
% - \Gls{label}   : nama istilah dengan huruf kapital di awal kalimat (contoh: \Gls{triplet})
% - \glspl{label} : bentuk jamak jika didefinisikan
...
% Istilah knowledge graph dan graph database
...
% Istilah ekstraksi informasi dan NLP
\newglossaryentry{nlp}
{
...
% Istilah temu balik informasi (information retrieval)
...
% Istilah deep learning dan machine learning
...
\newglossaryentry{pretraining}
{
	...
	description={tahap pelatihan awal model pada \textit{dataset} besar untuk mempelajari representasi umum sebelum dilakukan \textit{fine-tuning} untuk tugas spesifik}
}
\newglossaryentry{finetuning}
{
	name={\textit{fine-tuning}},
	description={proses melatih ulang model yang sudah di-\textit{pretrain} pada \textit{dataset} spesifik untuk menyesuaikan dengan tugas tertentu}
}
...
% Istilah graph neural network dan algoritma graf
\newglossaryentry{gnn}
{
...
% Istilah bibliometrik dan publikasi akademik
...
% Istilah pemrosesan data dan pipeline
...
\newglossaryentry{benchmark}
{
	...
	description={\textit{dataset} atau tugas standar yang digunakan untuk mengukur dan membandingkan performa model atau sistem}
}

% Istilah sistem dan antarmuka aplikasi
